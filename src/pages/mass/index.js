...
import { Typography, TextField, Select, MenuItem, Backdrop, Stack, Button, CircularProgress, Snackbar, Alert } from '@mui/material';
...
export default function Mass({ isAdmin }) {
    ...
    const { data, mutate } = useSWR('/api/requests', (url) => request.get(url).then((response) => response.data));
    ...
    return (
        ...
        <div className={styles.page}>
            ...
            <main className={styles.content}>
                ...
                <Typography className={styles.subtitle} variant="body1">Перечислите ID заказов <i>(через пробел)</i> и выберите что сделать.</Typography>
                <Typography className={styles.subtitle} variant="body1">Заявка отправится администратору и исполнится сразу после подтверждения.</Typography>
                ...
                <div className={styles.content}>
                    {Boolean(notFounded.length) && <Typography className={styles.warning} variant="body1"><b>Заказы не найдены:</b> {notFounded.join(', ')}</Typography>}
                    {Boolean(notNeeded.length) && <Typography className={styles.warning} variant="body1"><b>Уже в этом статусе:</b> {notNeeded.join(', ')}</Typography>}
                    {nothingToChange && <Typography className={styles.warning} variant="body1">Пустая заявка</Typography>}
