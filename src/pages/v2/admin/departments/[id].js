  ...
  return (
    <>
      <Typography variant="h6">
        ...
        <i>
          {department.name} [{TYPE_MAPPER[department.type]}]
        </i>
      </Typography>
      <form onSubmit={onSubmit}>
        ...
        {isGus && (
          <TextField
            name="priorityCapacity"
            label="Макс. кол-во приоритетных заявок"
            className={styles.input}
            defaultValue={department.priorityCapacity || 0}
            fullWidth
            type="number"
            required
          />
      ...
      {isGus && (
        <div className={styles.cities}>
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="Добавить город" />
            )}
          />
          <List className={styles.citiesList}>
            {department.cities.map((data) => (
              <City {...data} departmentID={department._id} update={update} />
            ))}
          </List>
        </div>
      )}
...
export const getServerSideProps = async ({ res }) => {
  ...
  return {
    props: {
      departmentID,
      fallback: {
        ["/api/admin/department/" + departmentID]: JSON.parse(
          JSON.stringify(res.locals.department)
        ),
        "/api/admin/unused-cities": JSON.parse(
          JSON.stringify(res.locals.cities)
        ),
      },
    },
  };
